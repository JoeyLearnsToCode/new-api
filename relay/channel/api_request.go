...
func applyHeaderOverridePlaceholders(template string, c *gin.Context, apiKey string) (string, bool, error) {
	...
	// 替换环境变量 ${env:KEY_NAME}
	template = envVariableRegex.ReplaceAllStringFunc(template, func(match string) string {
		parts := envVariableRegex.FindStringSubmatch(match)
		if len(parts) == 2 {
			keyName := parts[1]
			if val, exists := os.LookupEnv(keyName); exists && val != "" {
				return val
			}
		}
		return match
...
//   - ${env:<name>}: resolved to environment variable value
//
