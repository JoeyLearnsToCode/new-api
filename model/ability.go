...
func getPriority(group string, models []string, retry int) (int, error) {
	...
	err := DB.Model(&Ability{}).
		...
		Where(commonGroupCol+" = ? and model in ? and enabled = ?", group, models, commonTrueVal).
...
func getChannelQuery(group string, models []string, retry int) *gorm.DB {
	maxPrioritySubQuery := DB.Model(&Ability{}).Select("MAX(priority)").Where(commonGroupCol+" = ? and model in ? and enabled = ?", group, models, commonTrueVal)
	channelQuery := DB.Where(commonGroupCol+" = ? and model in ? and enabled = ? and priority = (?)", group, models, commonTrueVal, maxPrioritySubQuery)
	if retry != 0 {
		priority, err := getPriority(group, models, retry)
		if err != nil {
			common.SysError(fmt.Sprintf("Get priority failed: %s", err.Error()))
		} else {
			channelQuery = DB.Where(commonGroupCol+" = ? and model in ? and enabled = ? and priority = ?", group, models, commonTrueVal, priority)
		}
	}
